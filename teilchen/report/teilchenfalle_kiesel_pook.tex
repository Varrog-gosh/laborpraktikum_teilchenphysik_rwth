...
\begin{table}
	...
	\begin{tabular}{ c| c }
		...
		\hline
		 1000 & 5.07  ± 0.39 (stat.) ± 0.07 (sys.) \\
		 1000 & 1.01  ± 0.5  (stat.) ± 0.01 (sys.)   \\
		  375 & 1.14  ± 1.8 (stat.) ± 0.19 (sys.) \\
		  400 & 1.61  ± 1.74 (stat.) ± 0.21 (sys.)  \\
		  425 & 3.32  ± 0.73 (stat.) ± 0.04 (sys.)  \\
		  425 & 1.30  ± 0.68 (stat.) ± 0.17 (sys.)  \\
		  
		  	 %~ 1000 & 5.07 \cdot 10^{-3} ± 0.39 \cdot 10^{-3}(stat.) ± 0.07 \cdot 10^{-3}(sys.) \\
		 %~ 1000 & 1.01 \cdot 10^{-3} ± 0.5 \cdot 10^{-3} (stat.) ± 0.01 \cdot 10^{-3}(sys.)   \\
		  %~ 375 & 1.14 \cdot 10^{-2} ± 1.8 \cdot 10^{-3}(stat.) ± 0.19 \cdot 10^{-3}(sys.) \\
		  %~ 400 & 1.61 \cdot 10^{-2} ± 1.74 \cdot 10^{-3}(stat.) ± 0.21 \cdot 10^{-3}(sys.)  \\
		  %~ 425 & 3.32 \cdot 10^{-3} ± 0.73 \cdot 10^{-3}(stat.) ± 0.04 \cdot 10^{-3}(sys.)  \\
		  %~ 425 & 1.30 \cdot 10^{-2} ± 0.68 \cdot 10^{-3}(stat.) ± 0.17 \cdot 10^{-3}(sys.)  \\
	\end{tabular}
\caption{Das Verhältnis von Ladung zu Masse von verschiedenen Teichen, bestimmt durch die Kompensation der Gewichtskraft}
\label{tab:z-komp-result}
\end{table}
...
bei niedigen spannungen ist der relative fehler größer
immer schritte von 500 bar, also 250 = schwanken zwischen zwei schritten
für alle versuche ist $Ω = 2*pi*30 hz$
...
%~ \begin{align*}\label{result_compare}
 %~ \approx \unitfrac[ 1 ]{mC}{kg} \qquad\mathbf{ Z-Kompensation}\\
 %~ \approx \unitfrac[ 0.1 ]{mC}{kg} \qquad \mathbf{Resonanz}\\
 %~ \approx \unitfrac[ 0.001-0.01 ]{mC}{kg} \qquad \mathbf{Stabilität}\\
%~ \end{align}
\begin{align*}
	\begin{tabular}{ l r}
 \approx \unitfrac[ 1 ]{mC}{kg} & \mathbf{ Z-Kompensation}\\
 \approx \unitfrac[ 0.1 ]{mC}{kg} & \mathbf{Resonanz}\\
 \approx \unitfrac[ 0.001-0.01 ]{mC}{kg} & \mathbf{Stabilität}\\
	\end{tabular}
\end{align}
...
\begin{figure}[htb]
		\centering
		\includegraphics[height = 0.5\textheight]{../analyse/final_plot.pdf}\\
