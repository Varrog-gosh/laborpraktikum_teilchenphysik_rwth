...
\begin{table}[h]
	\centering
	\begin{tabular}{l| c | c }
		Medium &$U_{G,komp} [V] $ & $-\frac{q}{m}[\frac{mC}{kg}]$ \\
		\hline
		\multirow{3}{*}{Luft}&  50 ± 4(stat.) ± 14(sys.) &  5.95 ± 0.53(stat.) ± 1.67(sys.)  \\
		  &92 ± 8(stat.) ± 62(sys.) &  3.27 ± 0.28(stat.) ± 2.21(sys.)  \\
		  &273 ± 5(stat.) ± 55(sys.) & 1.09 ± 0.02(stat.) ± 0.23(sys.)  \\
		  \hline
		Vakuum &  68 ± 3(stat.) ± 27(sys.) &  4.40 ± 0.16(stat.) ± 1.76(sys.)  \\
	\end{tabular}
...
\begin{align*}
	...
	\text{Stabilität} &  \approx \unitfrac[ 0.04-0.5 ]{mC}{kg}\\ 
\end{align*}
Es ist dabei zu beachten, dass die einzelnen Messungen, die mit einer Messmethode durchgeführt wurden, knapp innerhalb ihrer Fehler
...
jeweilige Messmethode in der gleichen Größenordnungen wie die Messungen bei Normaldruck.
Es fehlen hier jedoch weitere Messungen bei niedrigeren Drücken, um eine Aussage
über die Auswirkungen der Luftreibung auf die Messung zu bestimmen.

Am ehesten vertrauen wir den Ergebnissen der Gravitationskompensation, da die Methode weniger Fehlerquellen als die anderen Methoden hat.
Die Stabilitätsmessung leidet unter dem schlechten Einschätzen der Grenzspannung, und für die Resonanz konnte nur eine Obergrenze festgestellt werden.

Eine Zusammenfassung aller Ergebnisse ist in Abbildung \ref{final_plot} dargestellt 
(man beachte die Überhöhung der dargestellten Fehler für die Stabilitätsmessung mit einem Faktor von 3).
\begin{figure}[htb]
		\centering
		\includegraphics[height = 0.8\textwidth]{../analyse/final_plot.pdf}\\
		\caption{Zusammenfassung der Ergebnisse für $\frac{q}{m}$ in logarithmische Auftragung, die Darstellung der Fehler für Stabilität wurden mit 3 multipliziert}
		\label{final_plot}
\end{figure}
\FloatBarrier % All figures, tables, which are buffered right now will be drawn befor next line
\subsection{Vergleich mit Milikanversuch}
Ein ähnliches Experiment wurde auch von Milikan durchgeführt.
1910 vermaß er die elektrische Ladung, indem er die elektrische Kraft von
geladene Öl-Tröpfchen durch die Kompensation der Erdbeschleunigung ausglich.
Die Masse wurde über die Viskosität bestimmt.

Bei dem durchgeführten Praktikumsversuch wird die Masse nicht gemessen, sondern muss abgeschätzt werden:
Es wird angenommen, dass der Durchmesser der Teilchen etwa $\unit[30]{μm}$ beträgt,
denn die Teichen sind unter Lichteinfall gerade noch mit dem Auge auflösbar.
Daraus ergibt sich eine Masse von $\unit[4\cdot 10^{-11}]{kg}$ für ein durchschnittliches Aluminiumteilchen.
Für die verschiedenen Methoden kann nun die Ladung ausgerechnet werden: Sie hat die Größenordnung von
$\unit[10^{-13}]{C} \approx \unit[10^6]{e}$ bis
$\unit[10^{-15}]{C} \approx \unit[10^4]{e}$.
Dies ist im Vergleich zu den Atomen im Teichen relativ wenig, so muss nur etwa jedes $10^{-10}$te Atom ionisiert werden.
Um die Elementarladung zu messen, muss nur die Auflösung (der Fehler) kleiner als eine Elementarladung sein, was in unserem Fall eine relative Unsicherheit von $10^{-6}$ benötigen würde.
Dies kann man zum Beispiel durch höhere Spannung erreichen, da dann Teilchen mit weniger Ladung auch gefangen werden können.
