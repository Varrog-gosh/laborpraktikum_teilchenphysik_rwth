...
\subsection{Bestimmung der Lebensdauern in Polyethylen}
	...
	der Gold Algorithmus \ref{gold_algo} benutzt. Hierbei wurde das gemessene Spektrum für $^{22}$Na als Faltung angenommen und das $^{60}$Co Spektrum als Responsefunktion des System
	gewählt. Die beiden Ausgangsfunktionen und die darraus resultierende Entfaltung sind in Abbildung \ref{fig:deconvolution} dargestellt. Es ist deutlich zu erkennen, dass die 
	Modifikation des ursprünglichen Signalspektrum durch die Auflösungsfunktion jenseits von Kanal 8000 also xx ns zu vernachlässigen ist. Da die Fortpflanzung der Fehler bei
	der Entfaltung nicht stattfindet werden die weiteren Anpassungen an den ursprünglichen Daten druchgeführt, die durchgeführte Entfaltung zeigt jedoch
...
\subsection{Vergleich der Messungen für Aluminum und Polyethylen}
	\begin{figure}
	\includegraphics[width=1.0\textwidth]{../analyse/compareHistos.pdf}
...
\begin{align*}
	F(t) &= \int_{-\infty}^\infty f(T)P(t-T)dT\\
	&= \int_0^\infty \frac{T}{τ}\exp\left( \frac{t}{τ} \right)\frac{1}{\sqrt{2π}σ}\exp\left( -\frac{(t-T-μ)^2}{2σ^2} \right)dT\\
	&=\\
	&= \frac{1}{2τ} \exp\left( \frac{2}{τ}\left( μ-t+\frac{σ^2}{τ} \right) \right) \frac{2}{\sqrt{π}}\int_{\frac{1}{\sqrt{2}σ}\left( μ-\frac{2σ^2}{τ} -t \right)}^\infty  e^{-γ^2}dγ\\
