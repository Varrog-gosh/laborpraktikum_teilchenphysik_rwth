...
Mit diesen Größen werden der elektroschwache Mischungswinkel berechnet und die Anzahl möglicher
Farbladungen $N_C$ eingeschränkt.
...
\begin{align*}
	...
	\not\mathrel{E}_i = \left| - \sum <\vec{E},\vec{e_i}> \right|
\end{align*}
...
\end{align*}
wird aus der Energie im Kalorimeter $E$ und der Pseudorapidität $\eta$ berechnet, wobei $\eta$ sowohl Informationen aus
...
W-Massen generiert, wie man in Abbildung \ref{fig:differentW} sehen kann.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{diffenentMcMasses.pdf}
	\caption{Die Verteilungen der W-Massen, die den Simulationen zu Grunde liegen. Es sind die
	maximal, minimale und mittlere W-Masse dargestellt.}
	\label{fig:differentW}
\end{figure}
...
vielen Stellen abweichen. Hierfür gibt es zwei mögliche Gründe: Die Simulation beschreibt die Wirklichkeit
...
und es wird versucht die Unterschiede durch geeignete Schnitte zu verringern. Ziel ist vor allem eine gute
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{chi2mwt.pdf}
...
Die Verschiebung in $\met$ und $E_T$ ergibt die selben Abweichungen und damit auch die selben Ergebnisse. Dies kommt durch die
Symmetrie von $\met$ und $E_T$ in der Definition von $m_T$.
Der gesamte Fehler für die Energieskalierung wird deshalb als der Einzelfehler von $\met$ und
$E_T$ abgeschätzt. Die Unsicherheit durch die Wahl der Selektionsgrenzen wird (auch wenn das sicherlich nicht ganz korrekt ist)
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=.8\textwidth]{mwt_el_etmet>0&&el_et>0tau.pdf}
	\includegraphics[width=.8\textwidth]{mwt_el_etmet>30&&el_et>30tau.pdf}
	\caption{Vergleich der Verteilungen von $m_T/E_T$ zwischen Daten und Simulation ohne kinematische Schnitte (oben) und
		mit Selektionsgrenzen $\met>\SI{30}{GeV}$ und $E_T>\SI{30}{GeV}$ (unten)}
...
\begin{figure}[htb]
	\centering
	\includegraphics[width=.8\textwidth]{chi2el_et.pdf}
...
von $E_T$ kommende Anteil den gesamten systematischen Fehler dominiert, da beide Größen korreliert sind
werden sie zunächst linear zu einem Gesamtfehler der Energieskala addiert bevor sie quadratisch mit dem Selektionsfehler addiert werden.
Es ergibt sich insgesamt

\begin{align*}
	m_W =  80.12 ± 0.33 (stat.)± 2.57 (sys.) \si{GeV}.
\end{align*}
...
instabiler sind, wird das Ergebnis von $m_T$ im Folgenden benutzt.
...
Da die, zur Berechnung genutzte, Formel nur die führende Ordnung der störungstheoretischen Rechnung beschreibt,
muss man das Ergebnis
...
\end{align*}
Der Weinbergwinkel weicht lediglich $0.002\sigma$ vom Literaturwert ab und ist mit diesem
...
in führender Ordnung nicht erzeugt werden, sondern nur paarweise, wie zum Beispiel in Abbildung \ref{fig:2Wfeynman} dargestellt. Deshalb wurde im LEP Experiment die W-Paarproduktion in den \cite{Achard:2005qy}
