...
Wahl der transversalen Energieskala zu $\SI{0.3}{MeV}$ bestimmt.
% du meinst sicherlich GeV???
% bei der verschiebung von 0 stimmt das nicht mit dem ergebnis von ./analyse überein und nicht mit
% dem was sonst da steht
\begin{table}[h]
	\centering
	\begin{tabular}{c| c c c}
		$\frac{\Delta E_{T}}{E_{T}}$ & $M_{W} [\si{GeV}]$ & $\Delta M_{W}$ &$\frac{\Delta M_{W}}{M_{W}}$\\
		\hline
		$2\%$ & $81.56\pm 0.26$ & 1.03 & $1.2\%$\\
		$0\%$ & $80.53\pm 0.3$ & 80.60 & $0\%$ \\
		$-2\%$ & $79.56\pm 0.3$ & 80.19 &$1.2\%$
		% ΔM_W braucht eine einheit
		% von was wird die differenz gebildet? müsste da nicht 0 in der mitte stehen?
	\end{tabular}
...
die Selektion hoch energetische Jets ausschließt und in der Berechnung von $\met$ die Unsicherheit
von $E_T$ eine dominante Rolle hat.
...
Wahl der Energieskala der fehlenden transversalen Energieskala zu $\SI{0.3}{MeV}$ bestimmt.
% auch hier: einheit richtig mev ist sehr wenig, wenn mev stimmt, dann solten wir explizit erwähnen,
% dass der fehler vernachlässigt werden kann
\begin{table}[h]
	\centering
	\begin{tabular}{c| c c c}
		$\frac{\Delta E_{T}}{E_{T}}$ & $M_{W} [\si{GeV}]$ & $\Delta M_{W}$ &$\frac{\Delta M_{W}}{M_{W}}$\\
		...
		$0\%$ & $80.53\pm 0.3$ & 80.60 & $0\%$ \\
		$-2\%$ & $79.56\pm 0.3$ & 80.19 &$1.2\%$
		% auch hier gilt das gleich wie in der tabelle oben
	\end{tabular}
...
und mit der partiellen Zerfallsbreite und einem linearen Faktor auf den gegebenen Literaturwert skaliert.
...
Der berechnete Wert weicht $0.1 \sigma$  vom Literaturwert $\sigma_{theo}=2.58 \pm \SI{0.09}{nb}$
\cite{versuchsanleitung} ab und ist somit kompatibel zu diesem.
Der hohe Fehler kommt hauptsächlich aus der Ungenauigkeit von $\mathcal{L}_\text{int}$.
...
Wirkungsquerschnitt zu berechnen. Dazu müsste das Integral in Gleichung \ref{form:xstot} mit passenden  Partondichtefunktionen (\textbf{pdf}) für verschiedene Werte
von $N_{\textsl{C}}$ in Gleichung \ref{form:xscms} integriert werden. Selbst diese Lösung wäre nur in führender Ordnung und wohl nicht geeignet um Vergleiche
